...
declare module '#app' {
  interface RuntimeNuxtHooks {
    ${endpointKeys.flatMap(i => [
      ...
      `'api-party:response:${i}': (option: FetchContext & { response: Response }) => HookResult`,
    ]).join('\n    ')}
  }
}
declare module 'nitropack/types' {
  interface NitroRuntimeHooks {
    ${endpointKeys.flatMap(i => [
      ...
      `'api-party:response:${i}': (option: FetchContext & { response: Response }, event?: H3Event) => HookResult`,
    ]).join('\n    ')}
  }
}
`.trimStart()
