...
export type RequestBody<T> = T extends { requestBody: { content: infer Body } } ?
    | (Body extends { 'application/octet-stream': any } ? { body: string | Blob; headers: { 'content-type': 'application/octet-stream' } } : {})
    | (Body extends { 'application/json': infer Schema } ? { body: Schema } : {})
    | (Body extends { 'application/x-www-form-urlencoded': any } ? { body: FormData; headers: { 'content-type': 'application/x-www-form-urlencoded' } } : {})
  : {}

export type Method<M extends IgnoreCase<HttpMethod>> = 'get' extends Lowercase<M> ? { method?: M } : { method: M }
...
export type APIRequestOptions<P extends PathItemObject, M extends IgnoreCase<keyof P & HttpMethod>> =
& Omit<
...
export type AllPaths<Paths extends Record<string, PathItemObject>> = RemovePrefix<keyof Paths & string, '/'>
